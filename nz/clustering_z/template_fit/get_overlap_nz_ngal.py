...
samples = [
	{
		'label':'rm_0.5.1_wide_0.9binning',
		'file':rmfile,
...
for sample in samples:
	...
	np.savetxt('output/n_gal_total_{label}.txt'.format(label=label), n_gal_total_list )
	np.savetxt('output/n_gal_overlap_{label}.txt'.format(label=label), n_gal_overlap_list )


	#make n(z)
	nz_table_total = cat.makenz_table(binedges, bins=200, useweights=True, sample=False, normed=False, usecol=sample['usecol'])
	...
	np.savetxt( 'output/nz_combined_total_{label}.txt'.format(label=label) , np.transpose(nz_table_total))
	np.savetxt( 'output/nz_combined_overlap_{label}.txt'.format(label=label) , np.transpose(nz_table_overlap))
	...
	plt.xlim([0.1,1.1])
	plt.savefig('plots/nz_redmagic_overlap_{label}.png'.format(label=label))
	plt.close()
